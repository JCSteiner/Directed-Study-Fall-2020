...
def loadData(filePathsSrc, dataFolderName = 'data', phePath = 'phenomes.txt',
    ...
    #records the maxes of the lengths of inputs and targets
    maxBin = 0
    maxTarget = 0
        ...
        for line in transcript:
            ...
            for word in wordList[1:]:
                  ...
                  # otherwise, appends the word to the list
                    ...
                    if verbose:
                        print('NO PHENOME FOUND FOR WORD:', word.upper())
            ...
            if len(tokenList) > maxTarget:
                maxTarget = len(tokenList)
            
            # appends this given input with its target
            data.append((mfcc, tokenList))
    ...
    token2Idx['<blank>'] = len(token2Idx)
    
    # returns the data, token2Idx dictionary, and phenome dictionary
    return data, token2Idx, phenomeDict, maxBin, maxTarget

def loadSingleAudio(filePath):
    
    # loads in the audio file
    audio, _ = lb.load(filePath, SAMPLE_RATE)
